...
namespace MiniTwitter.Models.Net
{
    class TwitterClient : OAuthBase
    {
        ...
        private ulong? _recentId;

        public Status[] RecentTimeline
        {
            get { return IsLogined ? GetStatuses("http://api.twitter.com/1/statuses/home_timeline.xml", new { count = 200 }, ref _recentId) : Statuses.Empty; }
        }

        private ulong? _repliesId;

        public Status[] RepliesTimeline
        {
            get { return IsLogined ? GetStatuses("http://api.twitter.com/1/statuses/mentions.xml", new { count = 100 }, ref _repliesId) : Statuses.Empty; }
        }

        private ulong? _archiveId;

        public Status[] ArchiveTimeline
        {
            get { return IsLogined ? GetStatuses("http://api.twitter.com/1/statuses/user_timeline.xml", new { count = 100 }, ref _archiveId) : Statuses.Empty; }
        }

        private ulong? _receivedId;

        public DirectMessage[] ReceivedMessages
        {
            get { return IsLogined ? GetMessages("http://api.twitter.com/1/direct_messages.xml", ref _receivedId) : DirectMessages.Empty; }
        }

        private ulong? _sentId;

        public DirectMessage[] SentMessages
        {
            get { return IsLogined ? GetMessages("http://api.twitter.com/1/direct_messages/sent.xml", ref _sentId) : DirectMessages.Empty; }
        }
        ...
        public bool CreateFollow(string screenName)
        {
            try
            {
                Post("http://api.twitter.com/1/friendships/create.xml", new { screen_name = screenName });
        ...
        public bool CreateBlock(string screenName)
        {
            try
            {
                Post("http://api.twitter.com/1/blocks/create.xml", new { screen_name = screenName });
        ...
        private bool DeleteBlock(string screenName)
        {
            try
            {
                Delete("http://api.twitter.com/1/blocks/destroy.xml", new { screen_name = screenName });
        ...
        public bool AddListUser(string name, int id, long userId)
        {
            try
            {
                Post(string.Format("http://api.twitter.com/1/{0}/{1}/members.xml", name, id), new { id = userId });
        ...
        public Status[] GetListStatuses(string name, ulong sinceId)
        {
            try
            {
                var screenName = LoginedUser.ScreenName;

                if (name.StartsWith("@"))
                {
                    int index = name.IndexOf('/');
                    screenName = name.Substring(1, index - 1);
                    name = name.Substring(index + 1);
                }

                var statuses = Get<Statuses>(string.Format("http://api.twitter.com/1/{0}/lists/{1}/statuses.xml", screenName, name), new { per_page = 200 }).Status;
